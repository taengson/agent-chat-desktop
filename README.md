...
장치 개인키는 운영체제 비밀 저장소에만 남고 프런트엔드에 반환되지 않습니다. TLS 개인키는 사용자가 지정한 PEM 파일에서 앱이 직접 읽으며, 앱 설정에는 파일 경로만 저장합니다. 외부 HTTPS 주소는 `https://` 도메인만 허용하며 raw IP, localhost, 사용자 정보, 쿼리, fragment는 저장하지 않습니다. 감사 로그는 이 PC에 최근 200건만 남고 전송하지 않으며, 초대 secret·키·요청 헤더·벤치마크 본문은 기록하지 않습니다.
