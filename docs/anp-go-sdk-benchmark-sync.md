...
| v2 UI·공개 설정·감사 로그 | 완료 | 장치 이름, 공개 fingerprint, 외부 HTTPS 도메인·인증서·개인키 파일 선택, TLS 개인키·CSR 생성, 수신 주소와 최근 200건의 로컬 감사 로그를 제공한다. |
| 직접 TLS 수신 endpoint | 완료 | 앱이 인증서 도메인·개인키 일치를 확인한 뒤 TLS 1.3 listener를 직접 열고, 상태 확인·서명된 페어링 요청·시작·중지와 앱 재시작을 처리한다. |
...
장치 개인키, prekey 비밀값, ratchet 상태는 `benchmark-sync-v2.json` 같은 일반 JSON 설정 파일에 평문 저장하지 않는다. 앱의 설정 폴더 권한은 보조 방어일 뿐, 키 보관소를 대체하지 않는다. TLS 개인키는 사용자가 선택하거나 앱이 사용자 지정 새 파일에 만든 PEM 파일에서 앱이 직접 읽으며, 설정 파일에는 그 경로만 저장한다.

### 4.2 ANP 장치 신원과 HTTPS TLS 키는 다르다

화면의 **장치 신원**은 ANP Ed25519/X25519 키다. v2 화면을 처음 열 때 OS 비밀 저장소에 한 번 자동 생성되므로, 사용자가 PEM 파일을 만들거나 경로를 지정할 필요가 없다. 이 키를 임의로 다시 만들면 이미 승인한 상대 장치와 결과 proof가 새 fingerprint를 신뢰하지 않으므로, 일반 화면에는 재생성 버튼을 두지 않는다.

반면 **직접 HTTPS 수신**의 인증서·개인키는 인터넷의 도메인을 증명하는 TLS 파일이다. 다음 흐름을 제공한다.

1. 외부 HTTPS 주소(예: `https://sync.example.com`)를 입력한다.
2. `TLS 키·CSR 만들기`를 눌러 개인키 PEM과 인증서 요청(CSR)을 각각 새 파일로 저장할 위치를 고른다. 기존 파일은 덮어쓰지 않으며 두 파일은 소유자 전용 권한으로 생성한다.
3. CSR을 공개 CA 또는 조직 CA에 제출해 해당 도메인의 인증서를 발급받는다.
4. 발급된 인증서 PEM과 방금 만든 개인키 PEM을 각각 파일 선택 창에서 고르고 수신을 시작한다.

앱은 자체 서명 인증서를 “연결 가능한 인증서”로 만들어 주지 않는다. 상대 PC는 표준 TLS 검증을 수행하므로, 실제 장치 간 연결에는 신뢰할 수 있는 CA 인증서가 필요하다.
